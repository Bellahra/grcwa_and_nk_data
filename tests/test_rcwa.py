...
def rcwa_assembly_pol(epgrid,freq,theta,phi,planewave,pthick,Pscale=1.):
...
def test_rcwa_pol():
    ...
    # p-polarization
    planewave={'p_amp':1,'s_amp':0,'p_phase':0,'s_phase':0}
    obj=rcwa_assembly_pol(epgrid,freq,theta,phi,planewave,pthick,Pscale=1.)
    R,T= obj.RT_Solve(normalize=0)
...
def test_pol_differs_from_fft():
    ...
    '''
    planewave={'p_amp':1,'s_amp':0,'p_phase':0,'s_phase':0}
    obj_fft=rcwa_assembly(epgrid,freq,theta,phi,planewave,pthick,Pscale=1.)
    _,T_fft= obj_fft.RT_Solve(normalize=0)
    obj_pol=rcwa_assembly_pol(epgrid,freq,theta,phi,planewave,pthick,Pscale=1.)
...
if AG_AVAILABLE:
    ...
    def test_epsgrad_pol():
        ...
        '''
        def fun(x):
            obj=rcwa_assembly_pol(x,freq,theta,phi,planewave,pthick,Pscale=1.)
            R,T= obj.RT_Solve(normalize=1)
            return R
    ...
    def test_thickgrad_pol():
        ...
        def fun(x):
            obj=rcwa_assembly_pol(epgrid.flatten(),freq,theta,phi,planewave,x,Pscale=1.)
            R,T= obj.RT_Solve(normalize=1)
            return R
